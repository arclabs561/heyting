...
//! but not over a faithful EL model.
//!
...
use heyting::conformal::{answer_set, calibrate, empirical_coverage};
use heyting::{AtomicScorer, Godel, Query, QueryConfig};

/// Reserved relation id for graded subsumption `C ⊑ ?`.
const SUB: usize = 0;

/// EL++ concept-box scorer: entities are boxes, the atomic relation is graded
/// subsumption via inclusion degree `exp(-‖relu(|cA-cB| + oA - oB)‖)`.
struct ElBoxScorer {
    centers: Vec<[f32; 2]>,
    offsets: Vec<[f32; 2]>,
}
...
impl ElBoxScorer {
    fn degree(&self, a: usize, b: usize) -> f32 {
        let mut acc = 0.0f32;
        for d in 0..2 {
            let v = ((self.centers[a][d] - self.centers[b][d]).abs() + self.offsets[a][d]
                - self.offsets[b][d])
                .max(0.0);
            acc += v * v;
        }
...
fn main() {
    ...
    let mut centers = vec![[0.0, 0.0]]; // Thing
    let mut offsets = vec![[6.5, 6.5]];
    let mid_pos = [[4.0, 0.0], [0.0, 4.0], [-4.0, 0.0], [0.0, -4.0]];
    for p in mid_pos {
        centers.push(p);
        offsets.push([3.5, 3.5]);
    }
    ...
    for (m, p) in mid_pos.iter().enumerate() {
        for s in [1.5f32, -1.5] {
            ...
            c[axis] += s;
            centers.push(c);
            offsets.push([1.2, 1.2]);
        }
    }
    let n = centers.len();
    let scorer = ElBoxScorer { centers, offsets };
    ...
    println!(
        ...
        scorer.degree(leaf, 1),
        scorer.degree(leaf, 0)
    );
    ...
    for &alpha in &[0.3f32, 0.2, 0.1] {
        let thr = calibrate::<Godel>(&scorer, &cal, &cfg, alpha).expect("calibrate");
        let cov = empirical_coverage::<Godel>(&scorer, &test, &cfg, &thr);
        println!(
    ...
    let thr = calibrate::<Godel>(&scorer, &cal, &cfg, 0.1).expect("calibrate");
    let set = answer_set::<Godel>(&scorer, &Query::anchor(leaf, SUB), &cfg, &thr);
